...
public class ArbolMViasBusqueda<K extends Comparable<K>, V> implements
    ...
    //AYUDA PARA LAINTERFAZ
      ...
      public List<V> recorridoEnInOrdenParaValores() {
        ...
        recorridoEnInOrdenParaValores(this.raiz, lista);
        return lista;
    }
     
    private void recorridoEnInOrdenParaValores(NodoMVias nodo, List<V> lista) {
        if (nodo != null) {
            
            for (int i = 0; i < nodo.nroDeClavesNoVacias(); i++) {
            recorridoEnInOrdenParaValores(nodo.getHijo(i), (List<V>) lista);
            lista.add((V) nodo.getValor(i));
        }
        recorridoEnInOrdenParaValores(nodo.getHijo(nodo.nroDeClavesNoVacias()), (List<V>) lista);
          
        }
    }

 public boolean editarCliente(K filaAEditar,V valorEditado) {
        if (!this.esArbolVacio()) {
        Queue<NodoMVias<K, V>> colaDeNodos = new LinkedList<>();
        colaDeNodos.offer(this.raiz);
        do {
